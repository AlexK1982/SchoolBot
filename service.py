...
def get_current_slot(
    schedule: list[dict], weekday: str, current_time: str
) -> Optional[dict]:
    weekday_normalized = normalize_weekday(weekday)
...
def format_current_slot(
    class_name: str, weekday: str, current_slot: Optional[dict]
) -> str:
    weekday_normalized = normalize_weekday(weekday)
    weekday_ru = WEEKDAY_RU.get(weekday_normalized, weekday_normalized)
...
def get_next_lesson(schedule, weekday, current_time):
    ...
    day_lessons.sort(
        key=lambda item: time_to_minutes(parse_time_range(item["time_range"])[0])
    )
